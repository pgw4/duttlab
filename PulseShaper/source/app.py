...
class appGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, nohardware=False):
        ...
        self.parameters = [50000, 300, 2000, 10, 10, 740, 10]  # should make into dictionary with keys 'sample', 'count time',
    ...
    def init_seq_text_box(self):
        f = open('./source/SeqDesigns/default.txt')
    ...
    def show_sequence(self):
        try:
            ...
            self.getReady()
            newparams = {'amplitude': 1000, 'pulsewidth': 50, 'SB freq': 0.01, 'IQ scale factor': 1.0, 'phase': 0.0,
    ...
    def start(self):
        ...
        if use_pts and enable_scan_pts:  # if scanning PTS freq
            self.x_arr = numpy.arange(start_freq, start_freq + step_freq * num_freq_steps, step_freq) #[:self.scan[2]] this was from old code, not sure why
